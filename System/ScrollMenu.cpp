...
void ScrollMenu::onTouchEnded(Touch *touch, Event *event) {
    ...
    if(this->_inMoving) {
        ...
        if ((touchPoint.x > (this->_scrollViewContentSize.x + this->_scrollViewPosition.x))
            || (touchPoint.x < this->_scrollViewPosition.x)
            || (touchPoint.y > (this->_scrollViewContentSize.y + this->_scrollViewPosition.y))
            || (touchPoint.y < this->_scrollViewPosition.y)) {
            // タッチ位置がScrollViewの範囲外の場合
            Menu::onTouchCancelled(touch, event);
        } else {
            Menu::onTouchEnded(touch, event);
        }
